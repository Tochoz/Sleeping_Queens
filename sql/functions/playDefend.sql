...
begin
    ...
    if not exists(select * from player_cards join cards on player_cards.id_card = cards.id_card where id_player=playerId and player_cards.id_card = cardId and (value='Рыцарь' or value='Волшебная Палочка')) then
        perform changeturn(roomid);
        call finishAttack(initPlayerId, true);
        return json_build_object(
                'status', 'invalid card',
